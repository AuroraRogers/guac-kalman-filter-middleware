...
static guac_kalman_filter* guac_kalman_filter_init(guac_socket* socket) {
    ...
    filter->layer_priorities = calloc(filter->max_layers, sizeof(filter->layer_priorities[0]));
    filter->layer_dependencies = calloc(filter->max_layers, sizeof(filter->layer_dependencies[0]));
    
    /* Initialize frequency statistics */
    filter->frequency_stats = calloc(filter->max_regions, sizeof(filter->frequency_stats[0]));
